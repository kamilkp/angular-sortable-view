...
;(function(window, angular){
		...
		return {
			...
			controller: ['$scope', '$attrs', '$interpolate', '$parse', function($scope, $attrs, $interpolate, $parse) {
				...
				this.$moveUpdate = function(opts, mouse, svElement, svOriginal, svPlaceholder, originatingPart, originatingIndex) {
					...
					var disableX = originatingPart.scope.$ctrl.disableX;
					// ----- move the element
					$helper[0].reposition({
						x: !disableX ? mouse.x + document.body.scrollLeft - mouse.offset.x * svRect.width : undefined,
	...
	module.directive('svDisabled', ['$parse', function($parse) {
		return {
			restrict: 'A',
			require: '^svPart',
			link: function($scope, $element, $attrs, $part) {
				if (!$attrs.svDisabled) throw new Error('no model provided');
				var model = $parse($attrs.svDisabled);

				$scope.$watch(model, function(value) {
					$part.isDisabled = !!value;
				});
			}
		};
	}]);

	module.directive('svDisableX', ['$parse', function($parse) {
		return {
			restrict: 'A',
			require: '^svPart',
			link: function($scope, $element, $attrs, $part) {
		...
		return {
			...
			link: function($scope, $element, $attrs, $controllers) {
				...
				function onMousedown(e) {
					...
					var disableX = $controllers[0].disableX;
					if (helper) {
						clone = helper.clone();
						clone.removeClass('ng-hide');
						if (!disableX) close.css('left', clientRect.left + document.body.scrollLeft + 'px');
						close.css('top', clientRect.top + document.body.scrollTop + 'px');
						target.addClass('sv-visibility-hidden');
					}
					else{
						clone = target.clone();
						clone.addClass('sv-helper');
						if (!disableX) clone.css('left', clientRect.left + document.body.scrollLeft + 'px');
						clone.css({
